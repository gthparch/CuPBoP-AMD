...
public:
  ...
  AddressSpaceCastPass() : FunctionPass(ID) {}
  ...
  virtual bool runOnFunction(Function &F) {
    ...
    for (FunctionType::param_iterator ii = FTy->param_begin(), ee = FTy->param_end();
          ii != ee; ++ii) {
    ...
    FunctionType *NFTy = FunctionType::get(FTy->getReturnType(), newParams, FTy->isVarArg());
    ...
    // llvm::PAListPtr PAL = F.getParamAttrs().addAttr(newParams.size(), llvm::ParamAttr::ByVal);
      ...
      for (BasicBlock::iterator BBI = FI->begin(); BBI != FI->end(); ++BBI) {
        ...
        if (llvm::AllocaInst *alloc = llvm::dyn_cast<llvm::AllocaInst>(inst)) {
          ...
          if (allocaType->isPointerTy()) {
            ...
            AllocaInst *new_arr = Builder.CreateAlloca(newType, nullptr, alloc->getName().str());
            new_arr->setAlignment(alloc->getAlign());

        
            // address space cast to ptr (generic address space of 0)

            auto *new_addr = Builder.CreateAddrSpaceCast(new_arr, allocaType);
        ...
        if (llvm::StoreInst *storeInst = llvm::dyn_cast<llvm::StoreInst>(inst)) {
          ...
          if (arg_set.find(sv) != arg_set.end()) {
            ...
            if (svType->isPointerTy()) {
              ...
              auto *new_addr = Builder.CreateAddrSpaceCast(sv, newType);
...
static RegisterPass<AddressSpaceCastPass>
    address_space_cast("address-space-cast",
