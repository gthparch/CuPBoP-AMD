   ...
   for (auto I = M.global_begin(), E = M.global_end(); I != E; ++I) {
    if (GlobalVariable *constant_memory = dyn_cast<GlobalVariable>(I)) {
      if (auto PT = dyn_cast<PointerType>(I->getType())) {
        ...
        if (AS == 3) {
          std::cout << " Shared Memory Variable: "  << constant_memory->getName().str() << std::endl;
          if (constant_memory->hasInternalLinkage()) {
            ...
            constant_memory->setVisibility(llvm::GlobalValue::VisibilityTypes::HiddenVisibility);
          }
