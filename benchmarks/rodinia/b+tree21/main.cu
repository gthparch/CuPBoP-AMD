...
#include <stdlib.h>
#include <sys/time.h> // (in directory
...
long transform_to_cuda(node *root, bool verbose) {
    ...
    while (queue != NULL) {
        ...
        if (!k->is_leaf) {
            ...
            for (i = 1; i < k->num_keys - 1; i++) {
                k->keys[i] = n->keys[i - 1];
                krecords[recordindex].value =
                    ((record *)n->pointers[i - 1])->value;
...
void usage_2(void) {
    ...
    printf("\tp <k> -- Print the path from the root to key k and its "
           "associated value.\n");
    ...
    printf("\tv -- Toggle output of pointer addresses (\"verbose\") in tree "
           "and leaves.\n");
...
node *insert_into_parent(node *root, node *left, int key, node *right) {
    ...
    return insert_into_node_after_splitting(root, parent, left_index, key,
                                            right);
}
...
int main(int argc, char **argv) {
    ...
    while (sscanf(commandPointer, "%c", &instruction) != EOF) {
        ...
        case 'k': {
            ...
            printf("\n ******command: k count=%d \n", count);
            if (count > 65535) {
                printf("ERROR: Number of requested querries should be 65,535 "
                       "at most. (limited by # of CUDA blocks)\n");
                exit(0);
            }
            ...
            long knodes_elem =
                ((long)(mem_used) - (long)rootLoc) / sizeof(knode);
