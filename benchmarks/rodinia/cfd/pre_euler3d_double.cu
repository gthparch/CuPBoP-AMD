...
#if block_length > 128
#warning                                                                       \
    "the kernels may fail too launch on some systems if the block length is too large"
#endif
...
compute_flux_contribution(double &density, double3 &momentum,
                          double &density_energy, double &pressure,
                          double3 &velocity, double3 &fc_momentum_x,
                          double3 &fc_momentum_y, double3 &fc_momentum_z,
...
__device__ inline void compute_velocity(double &density, double3 &momentum,
                                        double3 &velocity) {
...
__device__ inline double compute_speed_of_sound(double &density,
                                                double &pressure) {
    return sqrt(double(GAMMA) * pressure / density);
}

__global__ void cuda_compute_step_factor(int nelr, double *variables,
    ...
    // it all at once
    step_factors[i] =
        double(0.5) / (sqrt(areas[i]) * (sqrt(speed_sqd) + speed_of_sound));
}
...
__global__ void cuda_compute_flux_contributions(int nelr, double *variables,
...
#pragma unroll
    for (j = 0; j < NNB; j++) {
        ...
        normal_len = sqrt(normal.x * normal.x + normal.y * normal.y +
                          normal.z * normal.z);
...
int main(int argc, char **argv) {
    ...
    // set far field conditions and load them into constant memory on the gpu
    {
        ...
        compute_flux_contribution(h_ff_variable[VAR_DENSITY], h_ff_momentum,
                                  h_ff_variable[VAR_DENSITY_ENERGY],
                                  ff_pressure, ff_velocity, h_ff_fc_momentum_x,
                                  h_ff_fc_momentum_y, h_ff_fc_momentum_z,
        ...
        checkCudaErrors(cudaMemcpyToSymbol(
            ff_fc_momentum_x, &h_ff_fc_momentum_x, sizeof(double3)));
        checkCudaErrors(cudaMemcpyToSymbol(
            ff_fc_momentum_y, &h_ff_fc_momentum_y, sizeof(double3)));
        checkCudaErrors(cudaMemcpyToSymbol(
            ff_fc_momentum_z, &h_ff_fc_momentum_z, sizeof(double3)));

        checkCudaErrors(cudaMemcpyToSymbol(
            ff_fc_density_energy, &h_ff_fc_density_energy, sizeof(double3)));
    }
    int nel;
    int nelr;
