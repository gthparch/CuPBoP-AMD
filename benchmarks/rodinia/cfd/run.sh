...
for i in $(seq $count); do
    if [ "$FLAG" = "-r" ]; then
        ...
        { time (./$BENCHMARK.cupbop "$RODINIA_PATH/rodinia-data/cfd/fvcorr.domn.097K") >/dev/null; } 2>>$CuBoP_OUTPUT_FILE
        
        echo "[*] Running hipify version..."
        { time (./$BENCHMARK.hipify "$RODINIA_PATH/rodinia-data/cfd/fvcorr.domn.097K") >/dev/null; } 2>>$HIPIFY_OUTPUT_FILE

    else
        echo "[*] Running CuPBoP version..."
        time ./$BENCHMARK.cupbop "$RODINIA_PATH/rodinia-data/cfd/fvcorr.domn.097K"
        echo "[*] Running hipify version..."
        time ./$BENCHMARK.hipify "$RODINIA_PATH/rodinia-data/cfd/fvcorr.domn.097K"
    fi
done
