...
int main(int argc, char **argv) {
    ...
    // set far field conditions and load them into constant memory on the gpu
    {
        ...
        checkCudaErrors(cudaMemcpyToSymbol(
            ff_fc_momentum_x, &h_ff_fc_momentum_x, sizeof(float3)));
        checkCudaErrors(cudaMemcpyToSymbol(
            ff_fc_momentum_y, &h_ff_fc_momentum_y, sizeof(float3)));
        checkCudaErrors(cudaMemcpyToSymbol(
            ff_fc_momentum_z, &h_ff_fc_momentum_z, sizeof(float3)));

        checkCudaErrors(cudaMemcpyToSymbol(
            ff_fc_density_energy, &h_ff_fc_density_energy, sizeof(float3)));
    }
    int nel;
    int nelr;
