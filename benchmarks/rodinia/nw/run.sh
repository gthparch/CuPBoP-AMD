...
for i in $(seq $count); do
    if [ "$FLAG" = "-r" ]; then
        ...
        { time (./$BENCHMARK.cupbop 2048 10) >/dev/null; } 2>>$CuBoP_OUTPUT_FILE
        
        echo "[*] Running hipify version..."
        { time (./$BENCHMARK.hipify 2048 10) >/dev/null; } 2>>$HIPIFY_OUTPUT_FILE

    else
        echo "[*] Running CuPBoP version..."
        time ./$BENCHMARK.cupbop 2048 10
        echo "[*] Running hipify version..."
        time ./$BENCHMARK.hipify 2048 10
    fi
done
