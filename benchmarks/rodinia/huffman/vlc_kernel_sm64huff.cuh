 ...
 * Huffman encoding when using tree generation with restriction on the size
 * (max. Huffman codeword dize = 2x original symbol size).
 *
...
vlc_encode_kernel_sm64huff(unsigned int *data, const unsigned int *gm_codewords,
    ...
    // if (wrbits == 32) as[kc] = tmpcw32;				//unnecessary overhead; increases
    // number of branches else
