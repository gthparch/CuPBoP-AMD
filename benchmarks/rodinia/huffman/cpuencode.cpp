            ...
            while (numbits > 0) {
                ...
                if (numbits == writebits)
                    mask32 =
                        (cw32 & ((1 << numbits) - 1))
                        << (32 - startbit -
                            numbits); // first make sure that the start of the
                                      // word is clean, then shift to the left
                                      // as many places as you need
                else
                    mask32 = cw32 >>
                             (numbits -
                              writebits); // shift out the bits that can not fit
    ...
    *bitstreamPt = 0x00000000U;
    ...
    for (unsigned int k = 0; k < num_elements; k++) {
        ...
        for (unsigned int i = 0; i < 4; i++) {
            ...
            numbits += codewordlens[symbol];
            // if (numbits>32) printf("WARRNING! Element %d is combined into
            // numbits = %d!!!!!!!\n", k, numbits);
        }
