...
int runHisto(char *file, unsigned int *freq, unsigned int memSize,
    ...
    for (int i = 0; i < totalNum; i += partialNum * 2) {
        ...
        CHECK(cudaMemcpyAsync(dev_buffer0, buffer + i, partSize,
                              cudaMemcpyHostToDevice, stream0));
        CHECK(cudaMemcpyAsync(dev_buffer1, buffer + i + partialNum, partSize,
                              cudaMemcpyHostToDevice, stream1));

        // kernel launch - 2x the number of mps gave best timing
        histo_kernel<<<blocks * 2, 256, 0, stream0>>>(dev_buffer0, partSize,
                                                      dev_histo);
        histo_kernel<<<blocks * 2, 256, 0, stream1>>>(dev_buffer1, partSize,
                                                      dev_histo);
    }
