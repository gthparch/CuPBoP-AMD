...
//step is incremented by 1 time unit (h_init). 	4) Function assumes that time
//interval starts at 0 (xmin) and ends at integer value (xmax) specified by the
//uses as a parameter on command line.
...
int solver(fp **y, fp *x, int xmax, fp *params, fp *com,
    ...
    for (k = 1; k <= xmax; k++) { // start after initial value
        ...
        for (j = 0; j < ATTEMPTS; j++) {
            ...
            embedded_fehlberg_7_8(x[k], h, y[k - 1], y[k], err, params, com,

                                  d_initvalu, d_finavalu, d_params, d_com);

            //============================================================
            //		IF THERE WAS NO ERROR FOR ANY OF EQUATIONS, SET SCALE AND
            //LEAVE THE LOOP
            //============================================================
            ...
            // makes sense, as 1 is already the next time instance (added to
            // original algorithm)
            if (h >= 0.9) {
                h = 0.9;
            }
