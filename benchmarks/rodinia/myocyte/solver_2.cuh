...
__global__ void solver_2(int workload, int xmax, fp *x, fp *y, fp *params,
                         fp *com, fp *err, fp *scale, fp *yy, fp *initvalu_temp,
                         fp *finavalu_temp) {
    ...
    // save pointers, these pointers are one per workload, independent of time
    // step
    ...
    // value of the independent variable.  Set the value of the dependent
    ...
    if (tid < workload) {
        ...
        for (k = 1; k < (xmax + 1); k++) { // start after initial value
            ...
            y_pointer_initial =
                tid * ((xmax + 1) * EQUATIONS) + (k - 1) * EQUATIONS;
            y_pointer_current = tid * ((xmax + 1) * EQUATIONS) + k * EQUATIONS;
            x_pointer_current = tid * (xmax + 1) + k;

            x[x_pointer_current] =
                (fp)k; // set this to k if you want time incremente with respect
                       // to previous to be k+h, set this to k-1 if you want the
                       // increment to be h
            h = h_init;
            ...
            for (j = 0; j < ATTEMPTS; j++) {
                ...
                //============================================================
                //		IF THERE WAS NO ERROR FOR ANY OF EQUATIONS, SET
                // SCALE AND LEAVE THE LOOP
                //============================================================
                ...
                //============================================================
                //		FIGURE OUT SCALE AS THE MINIMUM OF COMPONENT
                // SCALES
                //============================================================
                ...
                for (i = 0; i < EQUATIONS; i++) {
                    ...
                    scale[scale_pointer + i] =
                        0.8 * pow(tolerance * yy[yy_pointer + i] /
                                      err[err_pointer + i],
                                  err_exponent);
                ...
                scale_fina =
                    min(max(scale_min, MIN_SCALE_FACTOR), MAX_SCALE_FACTOR);
                ...
                for (i = 0; i < EQUATIONS; i++) {
                    if (err[err_pointer + i] >
                        (tolerance * yy[yy_pointer + i])) {
                        outside = 1;
                    }
                }
                if (outside == 0) {
                ...
                // limit step to 0.9, because when it gets close to 1, it no
                // longer makes sense, as 1 is already the next time instance
            ...
            //==========================================================================================
            //		IF MAXIMUM NUMBER OF ATTEMPTS REACHED AND CANNOT GIVE
            // SOLUTION, EXIT PROGRAM WITH ERROR
            //==========================================================================================
