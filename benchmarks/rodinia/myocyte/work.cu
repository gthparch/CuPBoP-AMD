...
#include "embedded_fehlberg_7_8.cuh"
#include "solver.cuh"
...
int work(int xmax, int workload) {
    ...
    // y
    for (i = 0; i < workload; i++) {
        myocyte_read("../rodinia-data/myocyte/y.txt", y[i][0], EQUATIONS, 1, 0);
    }

    // params
    for (i = 0; i < workload; i++) {
        myocyte_read("../rodinia-data/myocyte/params.txt", params[i], PARAMETERS, 1, 0);
    }
    ...
    for (i = 0; i < workload; i++) {
        ...
        status = solver(y[i], x[i], xmax, params[i], com,
