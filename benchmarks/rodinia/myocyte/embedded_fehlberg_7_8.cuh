...
//     where                                                                  //
//     k1 = f( x[i],y[i] ),                                                   //
//     finavalu_temp[1] = f( x[i]+2h/27, y[i] + 2h*k1/27), // finavalu_temp[2] =
//     f( x[i]+h/9, y[i]+h/36*( k1 + 3 finavalu_temp[1]) ), // finavalu_temp[3]
//     = f( x[i]+h/6, y[i]+h/24*( k1 + 3 finavalu_temp[2]) ), //
//     finavalu_temp[4] = f( x[i]+5h/12, y[i]+h/48*(20 k1 - 75 finavalu_temp[2]
//     + 75 finavalu_temp[3])),                // finavalu_temp[5] = f(
//     x[i]+h/2, y[i]+h/20*( k1 + 5 finavalu_temp[3] + 4 finavalu_temp[4] ) ),
...
void embedded_fehlberg_7_8(fp timeinst, fp h, fp *initvalu, fp *finavalu,
                           fp *error, fp *parameter, fp *com,

                           fp *d_initvalu, fp *d_finavalu, fp *d_params,
                           fp *d_com) {
    ...
    master(timeinst_temp, initvalu_temp, parameter, finavalu_temp[0], com,
           ...
           d_initvalu, d_finavalu, d_params, d_com);
    ...
    for (i = 0; i < EQUATIONS; i++) {
        initvalu_temp[i] = initvalu[i] + h * (b41 * finavalu_temp[0][i] +
                                              b43 * finavalu_temp[2][i]);
    }
    ...
    for (i = 0; i < EQUATIONS; i++) {
        initvalu_temp[i] = initvalu[i] + h * (b51 * finavalu_temp[0][i] +
                                              b53 * finavalu_temp[2][i] +
    ...
    for (i = 0; i < EQUATIONS; i++) {
        initvalu_temp[i] = initvalu[i] + h * (b61 * finavalu_temp[0][i] +
                                              b64 * finavalu_temp[3][i] +
    ...
    for (i = 0; i < EQUATIONS; i++) {
        error[i] =
            fabs(err_factor * (finavalu_temp[0][i] + finavalu_temp[10][i] -
