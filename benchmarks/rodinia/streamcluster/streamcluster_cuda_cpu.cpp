...
#include "streamcluster_header.cuh"
...
// #define INSERT_WASTE				// Enables waste computation in dist
// function
...
int selectfeasible_fast(Points *points, int **feasible, int kmin, int pid,
    ...
    *feasible = (int *)malloc(numfeasible * sizeof(int));
    ...
    /*
       ...
       parallelized. When necessary, this can be parallelized by setting k1 and
       k2 to proper values and calling this routine from all threads ( it is
       called only by thread 0 for now ). Note that when parallelized, the
       randomization might not be the same and it might not be difficult to
       measure the parallel speed-up for the whole program.
     */
